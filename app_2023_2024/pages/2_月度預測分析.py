...
IMPORTANCE = ROOT / "reports" / "feature_importance_monthly_3way.csv"
ROLLING_BACKTEST = ROOT / "reports" / "monthly_2026h1_rolling_backtest_3way.csv"
FIXED_BACKTEST = ROOT / "reports" / "monthly_2026h1_fixed_backtest_3way.csv"
...
st.set_page_config(page_title="月度住房率預測（2023-2024訓練 2025測試）", page_icon="🗓️", layout="wide")
st.title("🗓️ 月度住房率預測分析（2023-2024訓練 2025測試）")
...
st.caption(
    f"已知資料範圍 {model_start:%Y-%m}～{model_end:%Y-%m}，共 {len(df):,} 筆"
    f"（模型訓練 {metadata.get('train_start', '2023-01')}～{metadata.get('train_end', '2024-12')}、"
    f"驗證選模 {metadata.get('val_start', '2025-01')}～{metadata.get('val_end', '2025-12')}、"
    f"盲測 {metadata.get('test_start', '2026-01')}～{metadata.get('test_end', '2026-06')}）；"
...
selected_model = metadata.get("selected_model", "Gradient Boosting")
selected_metrics = results.get("final_test", {}).get(selected_model, {})
validation_candidates = results.get("validation_selection", {})
...
with st.expander("模型評估：這個預測有多可信", expanded=True):
    model_version = metadata.get("model_version", "monthly-3way-v1")
    ...
    if selected_metrics:
        ...
        v3.metric("訓練／驗證選模／盲測", f"{metadata.get('train_start', '2023-01')}～{metadata.get('train_end', '2024-12')} / "
                  f"{metadata.get('val_start', '2025-01')}～{metadata.get('val_end', '2025-12')} / "
                  f"{metadata.get('test_start', '2026-01')}～{metadata.get('test_end', '2026-06')}")
        v4.metric("原始輸入整體缺失率", f"{overall_missing:.2f}%")
        st.caption(
            f"選模階段只用驗證集（{metadata.get('val_start', '2025-01')}～{metadata.get('val_end', '2025-12')}），"
            f"未觸碰盲測資料；選定後以訓練＋驗證全量（{metadata.get('refit_rows', 0):,} 筆）重新訓練，"
            f"再用時間外測試期間 {metadata.get('holdout_start', '2026-01')}～{metadata.get('holdout_end', '2026-06')} 做唯一一次盲測。"
        )
        st.caption(metadata.get("split_note", ""))
    ...
    if validation_candidates:
        comparison = pd.DataFrame(validation_candidates).T.reset_index().rename(
            columns={"index": "模型", "mae": "MAE", "rmse": "RMSE", "r2": "R²", "n": "驗證筆數"}
        )
        st.markdown(
            f"**候選模型比較（驗證集 {metadata.get('val_start', '2025-01')} 至 "
            f"{metadata.get('val_end', '2025-12')}，選模階段未碰盲測資料）**"
        )
        st.dataframe(comparison, width="stretch", hide_index=True)
    if results.get("final_test"):
        final_comparison = pd.DataFrame(results["final_test"]).T.reset_index().rename(
            columns={"index": "模型", "mae": "MAE", "rmse": "RMSE", "r2": "R²", "n": "盲測筆數"}
        )
        st.markdown(
            f"**最終盲測（{metadata.get('holdout_start', '2026-01')} 至 "
            f"{metadata.get('holdout_end', '2026-06')}，只算一次）**"
        )
